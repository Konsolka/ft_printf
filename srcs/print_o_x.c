...
/*   Updated: 2019/12/03 12:26:11 by mburl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		print_x(t_flags *flags, va_list args)
{
	...
	nb_size = size;
	printf("==size = %d  width = %d===", size, flags->width);
	if (flags->hash && (nb > 0 || flags->precision < 0))
		size += 2;
	size = (flags->precision > size) ? flags->precision : size;
	if (flags->zero && flags->hash == 1 && nb != 0 && !flags->minus)
		ft_write("0x", 2, flags);
	if (flags->width && !flags->minus)
		size = ft_pad(flags, size);
	if (flags->hash == 1 && nb != 0 && !flags->zero)
		ft_write("0x", 2, flags);
	while (nb_size++ < flags->precision)
		ft_write("0", 1, flags);
	if (flags->precision >= 0 || nb > 0)
